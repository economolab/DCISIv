...
scale = minimize_scalar(
    PSTH_scale_ob, args=[f_TP, f_TP_avg], method="bounded", bounds=[0, 100]
).x
f_TP = f_TP * scale

scale = minimize_scalar(
    PSTH_scale_ob, args=[f_FP, f_FP_avg], method="bounded", bounds=[0, 100]
).x

f_FP = scale * f_FP

ISI_v = SpikeSim.sim_ISI_v_inhomo(f_TP, f_FP, N=N, t_stop=t_stop)
pred_FDR = DCISIv(np.vstack([f_TP + f_FP, f_FP]), [ISI_v, 0], N=N).res["FDRs"]["mean"][
    0
]
