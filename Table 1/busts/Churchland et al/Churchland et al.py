# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 20:14:15 2023
...
mypath = r"E:\\FDR Predictions DATA\\Churchland et al"
...
for file in tqdm(onlyfiles):
    ...
    cue_times = nwb_read.trials['go_cue_time'].data[:]
    ...
    for unit in units:
        ...
        for cue in cue_times:
            ...
            n_spikes = len(temp)
            viols += sum(np.diff(temp) < 0.002)
            spikes += n_spikes
            aligned_spikes.extend(temp)
            ISIs.extend(np.diff(temp))
    ...
    pred = JV_utils.pred_FDR(np.stack(PSTHs_temp), ISI_viol_temp, tau=2, tau_c=0)
