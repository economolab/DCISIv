...
def sim_ISI_v_homo(f_t, FDR, t_stop=1000, tau=2.5, N=1):
    ...
    clu_TP = StationaryPoissonProcess(rate=f_TP, t_stop=t_stop, refractory_period=tau)

    clu_FPs = []
    for _ in range(N):
        clu_FPs.append(
            StationaryPoissonProcess(rate=f_FP, t_stop=t_stop, refractory_period=tau_FP)
        )
...
def sim_ISI_v_inhomo(f_TP, f_FP, T=6, tau=2.5, N=1, t_stop=1000):
    ...
    clu_TP = NonStationaryPoissonProcess(rate_signal=sig_TP, refractory_period=tau)

    clu_FPs = []
    for _ in range(N):
        clu_FPs.append(
            NonStationaryPoissonProcess(rate_signal=sig_FP, refractory_period=tau_FP)
        )
